...
import { Star, Calendar, Clock, User, Briefcase, CreditCard, MessageSquare } from "lucide-react";

const BookingDetailsModal = ({ isOpen, onClose, booking }) => {
  console.log("This is the details of the bookings: ", booking);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/60 p-4">
      <div className="bg-[#0d0d0d] border border-primary/20 rounded-lg shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#111] rounded p-3 border border-primary/10">
              ...
              <p className="text-white font-medium">
                {booking.clientId?.name?.first || "N/A"} {booking.clientId?.name?.last || ""}
              </p>
              <p className="text-gray-400 text-xs mt-1">{booking.clientId?.email || ""}</p>
            </div>
            ...
            <div className="bg-[#111] rounded p-3 border border-primary/10">
              ...
              <p className="text-white font-medium">{booking.therapistId?.title || "N/A"}</p>
            </div>
          </div>
          ...
          <div className="bg-[#111] rounded p-4 border border-primary/10">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-white font-medium">{booking.serviceId?.name || "N/A"}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-500 uppercase mb-1">Price</p>
                <p className="text-primary font-semibold text-lg">${booking.price?.amount || 0}</p>
              </div>
            </div>
          </div>
          ...
          {/* Review Section - Only show if reviewed */}
          {booking.isReviewed && booking.review && (
            <div className="bg-[#111] rounded p-4 border border-primary/10">
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-white text-sm font-medium">{booking.review.rating}/5</span>
              </div>
          ...
          <div className="bg-[#111] rounded p-3 border border-primary/10">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <p className="text-white capitalize">{booking.paymentMode || "N/A"}</p>
              </div>
              {/* <div> <p className="text-gray-500 text-xs">Settlement</p> */}
              {/*   <p className="text-white text-xs truncate"> */}
              {/*     {booking.settlementId || "N/A"} */}
              {/*   </p> */}
              {/* </div> */}
            </div>
          </div>
